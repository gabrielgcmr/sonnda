...
const createLabItems = `-- name: CreateLabItems :one
...
type CreateLabItemsParams struct {
...
func (q *Queries) CreateLabItems(ctx context.Context, arg CreateLabItemsParams) (pgtype.UUID, error) {
	row := q.db.QueryRow(ctx, createLabItems,
		arg.LabResultID,
		arg.ParameterName,
		arg.ResultValue,
		arg.ResultUnit,
...
const deleteLabReport = `-- name: DeleteLabReport :exec
...
// Deletes
// ============================================================
func (q *Queries) DeleteLabReport(ctx context.Context, id pgtype.UUID) error {
	_, err := q.db.Exec(ctx, deleteLabReport, id)
	return err
}
...
const existsLabReportByPatientAndRawText = `-- name: ExistsLabReportByPatientAndRawText :one
...
type ExistsLabReportByPatientAndRawTextParams struct {
	PatientID pgtype.UUID `json:"patient_id"`
	RawText   pgtype.Text `json:"raw_text"`
}
...
// ============================================================
func (q *Queries) ExistsLabReportByPatientAndRawText(ctx context.Context, arg ExistsLabReportByPatientAndRawTextParams) (bool, error) {
	row := q.db.QueryRow(ctx, existsLabReportByPatientAndRawText, arg.PatientID, arg.RawText)
	var exists bool
	err := row.Scan(&exists)
	return exists, err
}
...
SELECT
  lr.id           AS report_id,
  lres.id          AS lab_result_id,
  lti.id          AS item_id,
  lr.report_date  AS report_date,
  lres.test_name   AS test_name,
  lti.parameter_name,
  lti.result_value,
  lti.result_unit
FROM lab_result_items lti
JOIN lab_results lres ON lti.lab_result_id = lres.id
JOIN lab_reports      lr  ON lres.lab_report_id      = lr.id
