...
func (uc *CreateUserFromIdentity) Execute(
	...
	existingByEmail, err := uc.userRepo.FindByEmail(ctx, identity.Email)
	if err != nil {
		return nil, err
	}
	if existingByEmail != nil {
		...
		updated, err := uc.userRepo.UpdateAuthIdentity(
			ctx, existingByEmail.ID, provider, identity.Subject)
		if err != nil {
			return nil, err
		}
		return updated, nil
	...
	u := &domain.User{
		...
		Role:         domain.RolePatient, // paciente se auto-registra
		CreatedAt:    now,
		UpdatedAt:    now,
	}
