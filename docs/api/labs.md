...
**Exemplo (curl):**
```bash
curl -i "http://localhost:8080/api/v1/patients/018f3a2a-4c1a-7c5a-9d9e-2b7d8d9c3f11/labs?limit=100&offset=0" \
  -H "Authorization: Bearer <id_token>"
```

## Listar labs completos (GET /api/v1/patients/:id/labs/full)

Inclui `test_results` com itens detalhados.

**Exemplo (curl):**
```bash
curl -i "http://localhost:8080/api/v1/patients/018f3a2a-4c1a-7c5a-9d9e-2b7d8d9c3f11/labs/full?limit=100&offset=0" \
  -H "Authorization: Bearer <id_token>"
```

## Upload de laudo (POST /api/v1/patients/:id/labs/upload)

Upload multipart com campo `file` (PDF/JPEG/PNG).

**Exemplo (curl):**
```bash
curl -i -X POST http://localhost:8080/api/v1/patients/018f3a2a-4c1a-7c5a-9d9e-2b7d8d9c3f11/labs/upload \
  -H "Authorization: Bearer <id_token>" \
